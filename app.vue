...
const optionsArray = [
  ...
  {
    titel: "2) Choose the name's popularity",
    category: "popularity",
    buttons: [Popularity.TRENDY, Popularity.UNIQUE],
  },
  {
    titel: "3) Choose name's length",
    category: "length",
...
<template>
  <div class="container">
    ...
    <div class="options-container">
      <Option v-for="option of optionsArray" :key="option.title" :option="option" :options="options" />
      <button class="primary" @click="computeSelectedNames">Find Names</button>
    </div>
    <div class="cards-container">
      <CardName v-for="(name, index) of selectedNames" :name="name" @remove="removeName(index)"/>
    </div>
  </div>
</template>
